...
F = ((mse*36-resid.dev)/(resid.dev))/((24-8)/8)
F = round(F,2)
@
...
In the model we fit, we got deviance residual of \Sexpr{round(deviance(fit1),3)} on \Sexpr{df.residual(fit1)} degrees of freedom. Using these values we used the F-test to compare the models and find that our model is better than the one with individual pacakges as the F-statistic is \Sexpr{F}\\
...
@
The confidence interval obtained by Fieller's method for $log_2LD_{50}$ is: [\Sexpr{round(CI[1],3)}, \Sexpr{round(CI[2],3)}]
...
@
Bootstrap CI is calculated by obtaining \Sexpr{N} bootstrap samples and calculating $log_2LD_{50}$ estimate for each bootstrap sample and getting the 2.5\% and 97.5\% quantile for the estimates.\\
...
The confidence interval obtained by bootstrap method for $log_2LD_{50}$ is: [\Sexpr{round(CI.boot[1],3)}, \Sexpr{round(CI.boot[2],3)}]\\
...
and for the problem $\beta_0$ = (\Sexpr{beta0[1]},\Sexpr{beta0[2]}) and $\Sigma_0$ was defined with no covariance and 1 variance for both coefficients. First 100 samples were discarded as burnin.\\
Posterior mean estimate for $log_2LD_{50}$ obtained by Bayesian Probit linear model using the data augmentation technique by Albert and Chib is \Sexpr{post.mean}.\\
The credible interval obtained using the same technique for $log_2LD_{50}$ is: [\Sexpr{round(CI.bayes[1],3)}, \Sexpr{round(CI.bayes[2],3)}]\\
\section{Appendix}
Codes used:\\
\textbf{Problem 2}\\
